...
export default function CreatePost({ onPostCreated }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-5 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 mb-6 relative">
        ...
        <div className="flex gap-3 mt-3 pl-14 overflow-x-auto pb-2">
          {files.map((file, idx) => (
            <div key={idx} className="relative shrink-0">
               <img src={URL.createObjectURL(file)} alt="preview" className="w-20 h-20 object-cover rounded-lg border border-slate-200" />
               <button 
      ...
      {error && (
        <div className="mt-2 pl-14 text-red-500 text-sm">{error}</div>
      )}
