    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm mb-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Thống kê doanh thu từ phí dịch vụ 5% ({monthlyRevenues.length} tháng gần nhất)</p>
        </div>
        <div className="flex flex-col items-end">
          <div className="text-2xl font-bold text-[#372660]">{formatVND(systemRevenue)}</div>
          <p className="text-xs text-slate-400 mt-0.5">Tổng thanh toán: {formatVND(totalRevenue)}</p>
          {changePercent !== null && (
            <div className={`text-xs font-semibold flex items-center gap-1 mt-1 ${changePercent >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
