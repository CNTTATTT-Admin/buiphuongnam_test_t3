...
export default function MenteeBookingsPage() {
  ...
  const handleSubmitDispute = async (e) => {
    ...
    try {
      ...
      if (res.code === 1000) {
        setMessage({ type: 'success', text: 'Đã gửi yêu cầu khiếu nại thành công! Admin sẽ xem xét sớm nhất.' });
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      {disputeModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-slate-100">
              <h2 className="text-xl font-bold text-slate-800">Khiếu nại ca học</h2>
              <button onClick={() => setDisputeModal(null)} className="text-slate-400 hover:text-slate-600"><X className="w-6 h-6" /></button>
            </div>
            <form onSubmit={handleSubmitDispute} className="p-6 space-y-5">
              <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-xl text-sm mb-4">
                <strong>Lưu ý:</strong> Khiếu nại của bạn sẽ được gửi trực tiếp tới Admin để xem xét hoàn tiền. Bạn chỉ nên tạo khiếu nại nếu Mentor vắng mặt hoặc vi phạm nghiêm trọng trong buổi học.
              </div>

              {/* Reason */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wide">Nhập lý do khiếu nại (*)</label>
                <textarea
                  rows={4}
