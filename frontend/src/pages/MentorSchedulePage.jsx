...
export default function MentorSchedulePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (disputesRes.code === 1000)
        setMentorDisputes(disputesRes.result.content || []);
    } catch (error) {
  ...
  const handleCounterDispute = async (e) => {
    ...
    setCounterError("");
    try {
      const res = await disputeService.counterDispute(counterDisputeData.id, {
  ...
  const getStatusDisplay = (status) => {
    switch (status) {
      case "PENDING":
        return { text: "Chờ xác nhận", color: "bg-amber-100 text-amber-700" };
      case "PAID":
        return {
          text: "Đã thanh toán",
      ...
      case "CONFIRMED":
        return { text: "Đã xác nhận", color: "bg-blue-100 text-blue-700" };
      case "COMPLETED":
        return { text: "Đã hoàn thành", color: "bg-green-100 text-green-700" };
      case "REJECTED":
        return { text: "Đã từ chối", color: "bg-red-100 text-red-700" };
      case "CANCELLED":
        return { text: "Đã hủy", color: "bg-slate-100 text-slate-600" };
      case "DISPUTED":
        return {
          text: "Đang khiếu nại",
      ...
      case "REFUNDED":
        return { text: "Đã hoàn tiền", color: "bg-rose-100 text-rose-700" };
      default:
        ...
        <div className="mb-6 flex flex-col gap-4 sm:mb-8 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Quản lý thời gian và các ca học sắp tới của bạn.
            </p>
          </div>
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
            <button
              onClick={() => setShowDisputes(true)}
              className="relative flex w-full items-center justify-center gap-2 bg-white border border-orange-200 text-orange-600 px-4 py-2.5 rounded-lg text-sm font-semibold hover:bg-orange-50 transition-colors sm:w-auto"
            >
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 bg-[#372660] hover:bg-[#2b1d4c] text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors shadow-sm sm:w-auto"
            >
              <Calendar className="w-4 h-4" />
              <span className="sm:hidden">Cập nhật khung giờ</span>
              <span className="hidden sm:inline">Cập nhật khung giờ rảnh</span>
            </button>
          </div>
        </div>
          ...
          <div className="w-full lg:w-1/3 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              ...
              <h3 className="text-3xl font-bold text-slate-900 mb-2">
                {timeSlots.filter((s) => s.status === "AVAILABLE").length} slot
              </h3>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 hidden md:block select-none">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="text-slate-400 hover:text-[#372660] transition-colors"
                  >
              ...
              <div className="grid grid-cols-7 text-center gap-y-3 gap-x-1">
                {["CN", "T2", "T3", "T4", "T5", "T6", "T7"].map((d) => (
                  <div
                    key={d}
                    className="text-[10px] font-bold text-slate-400 mb-2"
                  >
                    {d}
                  </div>
                ))}
                ...
                {calendarDays.map((day, idx) => {
                  ...
                  const isToday =
                    getDateString(new Date().getDate()) === dateStr &&
              ...
              {selectedDate && (
                <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="text-[10px] uppercase font-bold text-[#372660] hover:underline"
                  >
                    Xem tất cả
                  </button>
                </div>
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                {timeSlots.length === 0 ? (
                  ...
                  timeSlots.map((slot) => (
                    <div
                      key={slot.id}
                      className="p-3 border border-slate-100 rounded-xl bg-slate-50 flex justify-between items-center transition-colors hover:border-[#372660]/30 group"
                    >
                      <div>
                        <p className="text-sm font-bold text-[#372660]">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="text-right">
                          <span
                            className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${slot.status === "AVAILABLE" ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}
                          >
          ...
          <div className="w-full lg:w-2/3">
            <div className="flex justify-between items-center mb-4 px-1">
              <h3 className="font-bold text-lg text-slate-900">
                {selectedDate
                  ? `Ca học ngày ${new Date(selectedDate).toLocaleDateString("vi-VN")}`
                  : "Danh sách Ca học"}
              </h3>
              ...
              {loading ? (
                ...
                >
                  ...
                  <p className="text-sm font-medium text-slate-400 mb-2">
                    Không có ca học nào{" "}
                    {selectedDate ? "trong ngày này" : "được đặt"}.
                  </p>
                  <p className="text-sm font-bold text-[#372660]">
                    Mở thêm khung giờ rảnh?
                  </p>
                </div>
              ) : (
                displayedBookings.map((cls) => {
                  ...
                  return (
                    <div
                      key={cls.id}
                      className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex flex-col sm:flex-row gap-5 items-start sm:items-center justify-between hover:shadow-md transition-shadow"
                    >
                      {/* User & Info */}
                      <div className="flex gap-4 items-center">
                        <div className="relative shrink-0">
                          <img
                            src={
                              cls.menteeAvatar ||
                              `https://ui-avatars.com/api/?name=${cls.menteeName}&background=random`
                            }
                            alt="Avatar"
                        ...
                        <div>
                          <div className="mb-1 flex flex-wrap items-center gap-2">
                            ...
                            <span
                              className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${statusInfo.color}`}
                            >
                              {statusInfo.text}
                            </span>
                          </div>
                          <p className="text-sm font-semibold text-[#372660] mb-2 line-clamp-1">
                            {cls.menteeNotes || "Không có ghi chú"}
                          </p>
                          <div className="flex flex-wrap gap-4 text-xs font-medium text-slate-500">
                            <span className="flex items-center gap-1.5 border border-slate-100 bg-slate-50 px-2 py-1 rounded-md">
                              <Calendar className="w-3.5 h-3.5" />{" "}
                      ...
                      <div className="w-full sm:w-auto mt-2 sm:mt-0 flex flex-col sm:flex-row gap-2">
                        {(cls.status === "PENDING" ||
                          ...
                          <>
                            <button
                              onClick={() =>
                                handleBookingAction(cls.id, "REJECT")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleBookingAction(cls.id, "CONFIRM")
                              }
                          ...
                          <>
                            {!cls.meetingLink ? (
                              <button
                                onClick={() => {
                                  const link = prompt(
                                    "Nhập link phòng học (Google Meet, Zoom...):",
                                  );
                              ...
                              >
                                <PlusCircle className="w-4 h-4" /> Thêm Link
                                Meet
                              </button>
                            ) : (
                              <a
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-600 bg-white p-1 rounded-md shadow-sm border border-slate-100"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateSlot} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#372660] focus:border-[#372660] outline-none text-sm"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#372660] focus:border-[#372660] outline-none text-sm bg-slate-50"
                  required
                />
              </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-orange-50">
              <h3 className="font-bold text-lg text-slate-900 flex items-center gap-2">
                <AlertTriangle className="w-5 h-5 text-orange-500" /> Tạo đơn
                khiếu nại
              </h3>
              <button
            ...
            <form onSubmit={handleSubmitDispute} className="p-6 space-y-4">
              ...
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                ...
                <p className="text-sm font-bold text-slate-800">
                  {disputeBooking.menteeName ||
                    `Học viên #${disputeBooking.menteeId}`}
                </p>
                <p className="text-xs text-slate-500 mt-1">
                  {formatDate(disputeBooking.startTime)} •{" "}
                  {formatTime(disputeBooking.startTime)} -{" "}
                  {formatTime(disputeBooking.endTime)}
                </p>
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">
      ...
      {showDisputes && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden max-h-[80vh] flex flex-col">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="font-bold text-lg text-slate-900 flex items-center gap-2">
                <MessageSquare className="w-5 h-5 text-orange-500" /> Đơn khiếu
                nại từ học viên
              </h3>
              <button
            ...
            <div className="p-4 space-y-3 overflow-y-auto flex-1">
              {mentorDisputes.length === 0 ? (
                <p className="text-sm text-slate-500 text-center py-6">
                  Chưa có đơn khiếu nại nào.
                </p>
              ) : (
                mentorDisputes.map((d) => (
                  <div
                    key={d.id}
                    className="p-4 border border-slate-100 rounded-xl bg-slate-50 space-y-2"
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-700 bg-white p-2 rounded-lg border border-slate-100">
                        {d.reason}
                      </p>
                    </div>
                    {d.counterReason ? (
                      <div className="mt-2">
                        ...
                        <p className="text-sm text-slate-700 bg-blue-50/50 p-2 rounded-lg border border-blue-100">
                          {d.counterReason}
                        </p>
                      </div>
                    ) : (
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-blue-50">
              <h3 className="font-bold text-lg text-slate-900 flex items-center gap-2">
                <MessageSquare className="w-5 h-5 text-blue-500" /> Kháng cáo
                quyết định
              </h3>
              <button
            ...
            <form onSubmit={handleCounterDispute} className="p-6 space-y-4">
              ...
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                <p className="text-xs text-slate-500 mb-1">
                  Lý do khiếu nại của học viên:
                </p>
                <p className="text-sm italic text-slate-700">
                  "{counterDisputeData.reason}"
                </p>
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">
